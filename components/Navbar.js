  ...
  return (
    <header className='sticky top-0 h-[4.5rem] w-full px-[4%] py-4 shadow-md z-30 bg-white lg:px-0'>
      <div className='flex items-center h-full w-full max-w-screen-lg mx-auto'>
        <div className='text-lg font-bold md:mr-6'>
          {/* Logo */}
          Hospid
        </div>
        <nav className='flex justify-between items-center h-full w-full'>
          <div className='flex items-center md:w-2/5 space-x-8'>
            <div className='hidden md:flex justify-around items-center w-full'>{children}</div>
          </div>
          ...
          <ButtonLink className='md:hidden' href='/login' secondary>
            Login
          </ButtonLink>
        </nav>
      </div>
