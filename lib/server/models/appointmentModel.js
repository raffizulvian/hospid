...
class Appointment {
  ...
  static async delete({ id }) {
    const APPOINTMENT_PATH = `appointments/${id}`;
  ...
  static async apply({ uid, username, aid }) {
    ...
    const addUser = registrantsRef.doc(registrantId).set(data);

    // Add new appointment to user list of appointments
    const userRef = db.doc(`roles/patient/users/${uid}`);
    const updateData = { appointments: FieldValue.arrayUnion(registrantsRef.doc(registrantId)) };
