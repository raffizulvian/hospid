...
class Appointment {
  static async create(doctor, desc, capacity) {
    ...
    const { id } = db.collection('appointments').doc();
  ...
  static async delete(id) {
    ...
    registrantsData.forEach(async (doc) => {
      ...
      await doc.data().ref.update({ appointments: FieldValue.arrayRemove(doc) });
    });

    // Remove all registrants in appointment list
    await deleteCollection(db, REGISTRANT_PATH, 8).catch((err) => {
      throw new AppError(err.message, 500);
    });
  ...
  static async apply(uid, username, aid) {
    ...
    await userRef
      .update({ appointments: FieldValue.arrayUnion(registrantsRef.doc(registrantId)) })
      .catch((err) => {
        throw new AppError(err.message, 500);
      });
  ...
  static async cancel(uid, aid) {
    const REGISTRANT_USER_PATH = `appointments/${aid}/registrants/${uid}`;

    const registrantUserRef = db.doc(REGISTRANT_USER_PATH);

    // Remove user from appointment registered patient
    await registrantUserRef.delete().catch((err) => {
      throw new AppError(err.message, 500);
    });
    ...
    await db
      ...
      .update({ appointments: FieldValue.arrayRemove(registrantUserRef) })
      .catch((err) => {
        throw new AppError(err.message, 500);
      });
