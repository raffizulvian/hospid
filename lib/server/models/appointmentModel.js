...
class Appointment {
  ...
  static async getRegistrants({ aid }) {
    ...
    registrants.forEach((doc) => {
      const user = doc.data().ref.get();
      users.push(user);
    });

    const usersResolved = await Promise.all(users).catch((err) => {
      throw AppError.BadGateway(err.message);
    });

    usersResolved.forEach((user) => {
      const { uid, firstName, lastName, age } = user.data();
      data.push({ uid, firstName, lastName, age });
    });
  ...
  static async apply({ uid, aid }) {
    ...
    const updateData = { appointments: FieldValue.arrayUnion(appointmentRef) };
