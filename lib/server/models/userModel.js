...
import { signAccessToken } from '../service/auth';
...
class User {
  ...
  static async signup(firstName, lastName, age, email, username, password) {
    ...
    // Data hashing
    const [encodedPassword, encodedEmail] = hash([password, email]);
  ...
  static async login(username, password, type) {
    ...
    const [isPasswordValid] = compare([password], [userPassword]);

    console.log(userPassword, isPasswordValid);
    ...
    const token = signAccessToken(responseData);
